...
contract XFlashToken is Ownable, ReentrancyGuard, ERC20("Flash escrowed token", "xFLASH"), IXFlashToken {
    ...
    struct RedeemInfo {
        uint256 flashAmount; // FLASH amount to receive when vesting has ended
    ...
    event Redeem(address indexed userAddress, uint256 xFlashAmount, uint256 flashAmount, uint256 duration);
    event FinalizeRedeem(address indexed userAddress, uint256 xFlashAmount, uint256 flashAmount);
    ...
    function redeem(uint256 xFlashAmount, uint256 duration) external nonReentrant {
        ...
        uint256 flashAmount = getFlashByVestingDuration(xFlashAmount, duration);
        emit Redeem(msg.sender, xFlashAmount, flashAmount, duration);
        ...
        if (duration > 0) {
            ...
            _finalizeRedeem(msg.sender, xFlashAmount, flashAmount);
        }
    }
    ...
    function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
        ...
        _finalizeRedeem(msg.sender, _redeem.xFlashAmount, _redeem.flashAmount);

        // handle dividends compensation if any was active
        if (_redeem.dividendsAllocation > 0) {
            // deallocate from dividends
            IXFlashTokenUsage(_redeem.dividendsAddress).deallocate(
                msg.sender,
                _redeem.dividendsAllocation,
                new bytes(0)
            );
    ...
    /**
     * @dev Finalizes the redeeming process for "userAddress" by transferring him "flashAmount" and removing "xFlashAmount" from supply
     *
     * Any vesting check should be ran before calling this
     * FLASH excess is automatically burnt
     */
    function _finalizeRedeem(address userAddress, uint256 xFlashAmount, uint256 flashAmount) internal {
        uint256 flashExcess = xFlashAmount.sub(flashAmount);

        // sends due FLASH tokens
        flashToken.safeTransfer(userAddress, flashAmount);

        // burns FLASH excess if any
        flashToken.burn(flashExcess);
        _burn(address(this), xFlashAmount);

        emit FinalizeRedeem(userAddress, xFlashAmount, flashAmount);
    }
