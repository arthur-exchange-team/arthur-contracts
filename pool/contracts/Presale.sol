...
contract Presale is Ownable, ReentrancyGuard {
  ...
  constructor(IERC20 flashToken, IXFlashToken xFlashToken, IERC20 saleToken, IERC20 lpToken, uint256 startTime, uint256 endTime, address treasury_) {
  ...
  event Claim(address indexed user, uint256 artAmount, uint256 xFlashAmount);
  ...
  modifier isSaleActive() {
    require(hasStarted() && !hasEnded() && FLASH.balanceOf(address(this)) >= MAX_FLASH_TO_DISTRIBUTE, "isActive: sale is not active");
    _;
  }
  ...
  */
  function artToDistribute() public view returns (uint256){
  ...
  * @dev Get user share times 1e5
    */
  function getExpectedClaimAmounts(address account) public view returns (uint256 artAmount, uint256 xFlashAmount) {
    if(totalAllocation == 0) return (0, 0);

    UserInfo memory user = userInfo[account];
    uint256 totalArtAmount = user.allocation.mul(artToDistribute()).div(totalAllocation);

    xFlashAmount = totalArtAmount.mul(XFLASH_SHARE).div(100);
    artAmount = totalArtAmount.sub(xFlashAmount);
  }
  ...
  function claim() external isClaimable {
    ...
    (uint256 artAmount, uint256 xFlashAmount) = getExpectedClaimAmounts(msg.sender);

    emit Claim(msg.sender, artAmount, xFlashAmount);
    ...
    _safeClaimTransfer(msg.sender, artAmount);
  }
  ...
  function burnUnsoldTokens() external onlyOwner {
    ...
    uint256 totalSold = artToDistribute();
  ...
  function _safeClaimTransfer(address to, uint256 amount) internal {
    uint256 artBalance = FLASH.balanceOf(address(this));
    bool transferSuccess = false;

    if (amount > artBalance) {
      transferSuccess = FLASH.transfer(to, artBalance);
    } else {
