...
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Flashpad staking position NFT", "spNFT") {
    ...
    struct StakingPosition {
        ...
        uint256 pendingXFlashRewards; // Not harvested xFlash rewards
        uint256 pendingFlashRewards; // Not harvested Flash rewards
    }
    ...
    uint256 public xFlashRewardsShare = 8000; // 80%, directly defines flashShare with the remaining value to 100%
    ...
    event SetXFlashRewardsShare(uint256 xFlashRewardsShare);
    ...
    function pendingRewards(uint256 tokenId) external view returns (uint256) {
        ...
        return
            position
                ...
                .add(position.pendingXFlashRewards)
                .add(position.pendingFlashRewards);
    }
     ...
     * Must only be called by the owner
     */
    function setXFlashRewardsShare(uint256 xFlashRewardsShare_) external {
        _requireOnlyOwner();
        require(xFlashRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

        xFlashRewardsShare = xFlashRewardsShare_;
        emit SetXFlashRewardsShare(xFlashRewardsShare_);
    }
    ...
    function createPosition(uint256 amount, uint256 lockDuration) external nonReentrant {
        ...
        _stakingPositions[currentTokenId] = StakingPosition({
            ...
            pendingFlashRewards: 0,
            pendingXFlashRewards: 0
        });
    ...
    function _harvestPosition(uint256 tokenId, address to) internal {
        ...
        // transfer rewards
        if (pending > 0 || position.pendingXFlashRewards > 0 || position.pendingFlashRewards > 0) {
            uint256 xFlashRewards = pending.mul(xFlashRewardsShare).div(_TOTAL_REWARDS_SHARES);
            uint256 flashAmount = pending.add(position.pendingFlashRewards).sub(xFlashRewards);

            xFlashRewards = xFlashRewards.add(position.pendingXFlashRewards);

            // Stack rewards in a buffer if to is equal to address(0)
            if (address(0) == to) {
                position.pendingXFlashRewards = xFlashRewards;
                position.pendingFlashRewards = flashAmount;
            } else {
                // convert and send xFLASH + FLASH rewards
                position.pendingXFlashRewards = 0;
                position.pendingFlashRewards = 0;

                if (xFlashRewards > 0) xFlashRewards = _safeConvertTo(to, xFlashRewards);
                // send share of FLASH rewards
                flashAmount = _safeRewardsTransfer(to, flashAmount);

                // forbidden to harvest if contract has not explicitly confirmed it handle it
                _checkOnNFTHarvest(to, tokenId, flashAmount, xFlashRewards);
            }
        }
     ...
     */
    function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 flashAmount, uint256 xFlashAmount) internal {
        address nftOwner = ERC721.ownerOf(tokenId);
